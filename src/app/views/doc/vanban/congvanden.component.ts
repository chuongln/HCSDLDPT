...
@Component({
  selector: 'app-congvanden',
  templateUrl: './vanban.component.html'
})
export class CongVanDenComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.onSearch();
    if (!this.permission.DH_VB_CongVanDen._view) {
  ...
  getDataTypes() {
    this.dmService.getDataType({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (res.error) {
  ...
  getVanBan() {
    this.docService.getListVanBan(this.searchForm.value, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (res.error) {
        this.common.messageErr(res);
  ...
  openDeleteDialog(data: any): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.docService.deleteVanBan({ id: data.id }, this.common.DH_VB_CongVanDen.Delete).subscribe(res => {
          if (res.error) {
            this.common.messageErr(res);
          } else {
            this.onSearch();
  ...
  loadDm() {
    this.docService.getCoQuanBanHanh({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (!res.error) {
        res.data.forEach(element => {
    ...
    this.docService.getHinhThucGuiNhan({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (!res.error) {
        res.data.forEach(element => {
    ...
    this.docService.getLinhVucVanBan({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (!res.error) {
        res.data.forEach(element => {
    ...
    this.docService.getLoaiVanBan({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (!res.error) {
        res.data.forEach(element => {
    ...
    this.docService.getMucDoKhan({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (!res.error) {
        res.data.forEach(element => {
    ...
    this.docService.getMucDoMat({}, this.common.DH_VB_CongVanDen.View).subscribe(res => {
      if (!res.error) {
        res.data.forEach(element => {
