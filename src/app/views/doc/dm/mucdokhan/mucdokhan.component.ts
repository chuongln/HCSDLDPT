...
@Component({
  selector: 'app-mucdokhan',
  templateUrl: './mucdokhan.component.html'
})
export class MucDoKhanComponent implements OnInit {
  ...
  ngOnInit() {
    this.onSearch();
    if (!this.permission.DH_VB_MucDoKhan._view) {
  ...
  //#region getMucDoKhan ((1))
  getMucDoKhan() {
    this.docService.getMucDoKhan({}, this.common.DH_VB_MucDoKhan.View).subscribe(res => {
      if (res.error) {
  ...
  openDeleteDialog(data: any): void {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.docService.deleteMucDoKhan({ id: data.id }, this.common.DH_VB_MucDoKhan.Delete)
          .subscribe(res => {
            if (res.error) {
              this.common.messageErr(res);
            } else {
